...
10. **Apply the brand brief** — use every section of the markdown returned by `xseek brand-context` in step 2. Treat it as a single voice spec; missing sections are fine, present sections are non-negotiable.
   ...
   - **Products to recommend**: if the brand brief has a `## Products to recommend` section (a B2C / own-domain catalog), once you know the article's region/topic run `xseek products <website> --region <region>` (or `--category <category>`) and recommend **3–8 specific products by name, each linked to its own page**. A specific product beats a generic category page, and it is the concrete way to satisfy the own-domain linking rule from Phase 2b. Never invent products that aren't returned by `xseek products`.
